    ...
    def _validate_knowledge_instance_names(self) -> None:
        ...
        for knowledge in self.knowledge_instances:
            ...
            if key in seen_combinations:
                duplicates.append(f"{knowledge_name} (db={db_id}, table={table_name})")
            else:
                seen_combinations[key] = f"db_id={db_id}, table={table_name}"

        if duplicates:
            unique_duplicates = list(set(duplicates))
            error_msg = (
                f"Duplicate knowledge instance configurations detected: {unique_duplicates}. "
                "Multiple knowledge instances with the same name, database, and table will cause "
                "content isolation issues. Either use different names, different databases, or different tables."
            )
            log_error(error_msg)
