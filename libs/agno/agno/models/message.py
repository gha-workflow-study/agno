...
from agno.metrics import RunMetrics as Metrics  # noqa: F401 — backward compat re-export
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Message":
        ...
        if "metrics" in data and data["metrics"] is not None:
            if isinstance(data["metrics"], dict):
                ...
                valid_keys = {f.name for f in dataclass_fields(MessageMetrics)}
                cleaned = {k: v for k, v in data["metrics"].items() if k in valid_keys}
                data["metrics"] = MessageMetrics(**cleaned)
