...
# Fields to exclude from serialization (internal utilities, not data)
_INTERNAL_FIELDS = {"timer"}
...
class ModelMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        valid_fields = {f.name for f in dc_fields(self)} - _INTERNAL_FIELDS
...
class SessionModelMetrics(ModelMetrics):
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SessionModelMetrics":
        valid = {f.name for f in dc_fields(cls)}
        return cls(**{k: v for k, v in data.items() if k in valid})

    @classmethod
    def from_model_metrics(cls, mm: "ModelMetrics", duration: Optional[float] = None, total_runs: int = 1) -> "SessionModelMetrics":
        return cls(
            id=mm.id,
            ...
            audio_input_tokens=mm.audio_input_tokens,
            audio_output_tokens=mm.audio_output_tokens,
            audio_total_tokens=mm.audio_total_tokens,
            cache_read_tokens=mm.cache_read_tokens,
            cache_write_tokens=mm.cache_write_tokens,
            reasoning_tokens=mm.reasoning_tokens,
            cost=mm.cost,
    ...
    def accumulate(self, other: "ModelMetrics") -> None:
        """Accumulate token counts from another ModelMetrics/SessionModelMetrics into this one."""
        ...
        if isinstance(other, SessionModelMetrics):
            ...
            if other.average_duration is not None:
                if self.average_duration is None:
                    ...
                    total_dur = (self.average_duration * old_runs) + (other.average_duration * other.total_runs)
                    self.average_duration = total_dur / self.total_runs if self.total_runs > 0 else None
...
class RunMetrics:
    ...
    def __add__(self, other: "RunMetrics") -> "RunMetrics":
        ...
        if self.additional_metrics is not None or other.additional_metrics is not None:
...
class SessionMetrics(RunMetrics):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if metrics_dict.get("details") is not None:
            valid_smm_fields = {f.name for f in dc_fields(SessionModelMetrics)}
            details_list = [
                m.to_dict()
                if isinstance(m, SessionModelMetrics)
                else {k: v for k, v in m.items() if k in valid_smm_fields and v is not None}
    ...
    def __add__(self, other: "SessionMetrics") -> "SessionMetrics":
        ...
        if self.details or other.details:
            ...
            for source_details in (self.details, other.details):
                if source_details:
        ...
        if self.provider_metrics is not None or other.provider_metrics is not None:
            ...
            if other.provider_metrics:
                merged_provider_metrics.update(other.provider_metrics)
        ...
        if self.additional_metrics is not None or other.additional_metrics is not None:
            ...
            if other.additional_metrics:
                merged_additional_metrics.update(other.additional_metrics)
